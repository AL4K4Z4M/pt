...
// Migration: Add last_ip column to users table if it doesn't exist
(async () => {
    try {
        const connection = await db.getConnection();
        const [columns] = await connection.query(
            `SELECT * FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = 'users' AND column_name = 'last_ip'`
        );
        if (columns.length === 0) {
            console.log('Adding "last_ip" column to "users" table...');
            await connection.query('ALTER TABLE users ADD COLUMN last_ip VARCHAR(45) NULL DEFAULT NULL');
            console.log('"last_ip" column added successfully.');
        }
        connection.release();
    } catch (err) {
        console.error('Failed to run migration for last_ip column:', err);
    }
})();

// Migration: Add ip_address column to banned_users table if it doesn't exist
(async () => {
    try {
        const connection = await db.getConnection();
        const [columns] = await connection.query(
            `SELECT * FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = 'banned_users' AND column_name = 'ip_address'`
        );
        if (columns.length === 0) {
            console.log('Adding "ip_address" column to "banned_users" table...');
            await connection.query('ALTER TABLE banned_users ADD COLUMN ip_address VARCHAR(45) NULL DEFAULT NULL');
            console.log('"ip_address" column added successfully.');
        }
        connection.release();
    } catch (err) {
        console.error('Failed to run migration for ip_address column:', err);
    }
})();
...
app.post('/api/users/login', async (req, res) => {
    ...
    try {
        const [rows] = await db.query(
            'SELECT id, username, email, password, is_admin, last_login_date, consecutive_login_days, last_ip FROM users WHERE username = ?',
            [username]
        );
        const user = rows[0];
        ...
        if (match) {
            ...
            // Asynchronous badge and IP logging logic post-response
            (async () => {
                try {
